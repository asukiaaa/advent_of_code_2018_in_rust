...
fn main() {
    ...
    let mut most_slept_guard_keep_sleep_counts = vec![];
    for (guard_id, date_flags) in &guard_flags {
        ...
        if slept_minutes > most_slept_guard_slept_minutes {
            ...
            most_slept_guard_keep_sleep_counts = keep_sleep_counts;
        }
    }
    // print_flags(most_slept_guard_keep_sleep_flags);
    println!("{:?} total {} minutes of guard {}", most_slept_guard_keep_sleep_counts, most_slept_guard_slept_minutes, most_slept_guard_id);
    let mut max_count = 0;
    let mut max_minute = 0;
    for (i, &c) in most_slept_guard_keep_sleep_counts.iter().enumerate() {
        if c > max_count {
            max_count = c;
            max_minute = i
        }
    }
    println!("Result: {} * {} = {}", most_slept_guard_id, max_minute, most_slept_guard_id * max_minute as i32);
