...
impl Field {
    ...
    fn unit_move_step(&mut self, key: &usize, step: &char) {
        ...
        let this_unit = self.units.get(key).unwrap();
    ...
    fn unit_attack(&mut self, key: &usize) {
        let mut this_unit = self.units.get(key).unwrap().clone();
        ...
        let hp = self.units.get(&target_key).unwrap().hp.clone().saturating_sub(3);
    ...
    fn finished(&self) -> bool {
        let mut e_count = 0;
        let mut g_count = 0;
        for (key, u) in self.units.iter() {
            if u.hp == 0 { continue }
            if u.role == 'E' { e_count += 1 }
            else { g_count += 1 }
        }
        e_count == 0 || g_count == 0
    }
...
fn main() {
    ...
    let mut field = Field::new(lines.len(), lines[0].len() - 2);
    for (y, line) in lines.iter_mut().enumerate() {
        ...
        for (x, cell_u8) in line.as_bytes().iter().enumerate() {
            ...
            if cell == 'G' || cell == 'E' {
                let units_len = field.units.len();
                field.set_as_unit(x, y, Unit::new(cell));
            } else if cell == '#' {
                field.set_as_wall(x, y);
            }
        }
    }
    field.print();
    ...
    println!("Result: {} * {} = {}", total_hp, loop_count, total_hp * loop_count);
}
